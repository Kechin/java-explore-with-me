...
public interface HitsRepository extends JpaRepository<Hit, Long> {
    ...
    @Query(value = "select new  ru.practicum.server.Stat (e.app.appName,e.ip, e.uri, count(e.ip)) " +
    ...
    @Query(value = "select new  ru.practicum.server.Stat (e.app.appName,e.ip, e.uri, count(distinct e.ip))  " +
    ...
    @Query(value = "select new  ru.practicum.server.Stat (e.app.appName,e.ip, e.uri, count(e.app)) " +
